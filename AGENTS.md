...
**Deploys:** `www` deploys itself via Workers Builds on every push to `main` (PRs get preview builds). Never run `bun run deploy` by hand. The production deploy command ends with `bun run purge`: HTML is edge-cached for an hour and each deploy drops the previous build's hashed assets, so the cache is purged or stale pages load without their CSS/JS (`CLOUDFLARE_PURGE_TOKEN`, a Workers Builds secret). The DO worker is not on Workers Builds — after changing anything under `workers/room`, a DO class, or `src/lib/fleet.ts` (the DO bundles its own copy: `HOSTS` roles/specs, the `SERVICES` whitelist, thresholds), run `bun run deploy:room` yourself, and do it *before* merging if `www` binds to a new class.
